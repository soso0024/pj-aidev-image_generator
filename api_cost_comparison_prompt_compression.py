...
percentages = [-6.8, 23.3, 0.4]  # Percentage changes vs base method
...
# Colors - Blue for decrease, Red for increases
colors = ["#3498DB", "#E74C3C", "#E74C3C"]  # Blue, Red, Red
...
for i, (bar, percentage) in enumerate(zip(bars, relative_costs)):
    if models[i] == "Claude 3.7 Sonnet":  # Special positioning for Claude
        # Position label above the bar with +6 offset
        ax.text(
...
plt.tight_layout()
plt.savefig(
    "imgs/api_cost_comparison_prompt_compression.png", dpi=300, bbox_inches="tight"
)
plt.show()
